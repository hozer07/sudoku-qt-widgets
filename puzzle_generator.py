...
def printCommaSeparated(sudoku):
    print(', '.join(str(cell.returnSolved()) for cell in sudoku))
...
def main(level):
    ...
    if level == 'Easy':
        p = perfectSudoku()
        answerKey = copy.deepcopy(p)
        s = puzzleGen(p)
        if s[2] != 'Easy':
            return main(level)
        else:
            print("Complete Puzzle #")
            printCommaSeparated(answerKey)
        ...
        print("Unsolved, level: " + str(s[2]), "#")
        printCommaSeparated(s[0])
        return 0
    if level == 'Medium':
        p = perfectSudoku()
        answerKey = copy.deepcopy(p)
